# Weapon Detection & Classification

Based on GardaSec idea, we build this for our Smart Security Cam Devices (SSCD).



*KALO BINGUNG DI BAGIAN TERTENTU, CEK DOCUMENTATION (coming soon)

## Instruksi Tugas

Kalian masing masing code untuk klasifikasi di `(coming soon, blm jadi)` dengan nama file sbb: (Pilih salah satu)

1. Pistol
2. Assault Rifle
3. Rifle

## Opsional

Nanti kita buat GUI, jadi pas presentasi bisa testing langsung.
